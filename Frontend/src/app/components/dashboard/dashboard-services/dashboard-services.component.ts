...
export class DashboardServicesComponent implements OnInit, OnDestroy {
  ...
  applyFilters(): void {
    this.filteredServices = this.services.filter(service => {
      ...
      const matchesLaboratory = this.selectedLaboratory
        ? (service.laboratory_id === Number(this.selectedLaboratory) || service.laboratory_name === this.getLabNameById(Number(this.selectedLaboratory)))
        : true;

      return matchesSearch && matchesCategory && matchesProvider && matchesLaboratory;
    });
  }
    ...
    saveEdit() {
      ...
      const payload = {
        ...
        model: this.editService.model,
        manufacturer: this.editService.manufacturer,
        requires_training: this.editService.requires_training,
        status: this.editService.status,
        laboratory_id: this.editService.laboratory_id
      };

      this.serviceService.updateService(this.editService.id, payload).subscribe({
        next: (res: any) => {
          if (res?.success) {
            // actualizar la lista localmente
            const idx = this.services.findIndex(s => s.id === this.editService.id);
            if (idx !== -1) {
    ...
    confirmDelete() {
      if (!this.editService?.id) return;
      const ok = confirm('¿Estás seguro de eliminar este servicio? Esta acción no se puede deshacer.');
      if (!ok) return;
      this.serviceService.deleteService(this.editService.id).subscribe({
        next: (res: any) => {
          if (res?.success) {
            // remover de la lista
